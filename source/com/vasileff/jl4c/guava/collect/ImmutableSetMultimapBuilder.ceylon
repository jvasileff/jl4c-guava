...
class ImmutableSetMultimapBuilder<Key, Item>()
    ...
    shared
    ImmutableSetMultimapBuilder<Key, Item> put(Key key, Item item) {
        delegate.put(key, item);
        return this;
    }

    shared
    ImmutableSetMultimapBuilder<Key, Item> putMultiple(Key key, Item* items) {
        for (item in items) {
            delegate.put(key, item);
        }
        return this;
    ...
    shared
    ImmutableSetMultimapBuilder<Key, Item> putAll({<Key->Item>*} entries) {
    ...
    shared
    ImmutableSetMultimapBuilder<Key, Item> orderKeysBy(compare) {
    ...
    shared
    ImmutableSetMultimapBuilder<Key, Item> orderItemsBy(compare) {
